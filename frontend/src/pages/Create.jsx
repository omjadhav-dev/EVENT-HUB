...
function Create() {
  ...
  const handleSubmit = (e) => {
    ...
    if (!userData) {
      alert("Please log in as an organizer to publish an event.");
      navigate("/login");
      return;
    }

    // Build a full event object shaped like the entries in
    // src/data/eventList.js, so it can be rendered by the exact same
    // <Card /> / Event.jsx / MyEvents.jsx components as the dummy data.
    const newEvent = {
      id: Date.now(),
      title,
      description,
      ...
      type: "Free",
      mode,
      // input[type=file] can't be controlled/stored directly - turn the
      // File into a blob URL so it can be used as an <img src>.
      ...
      host: userData?.name || "You",
      organizerEmail: userData?.email || null,
      attendees: 0,
      slots: 100,
      tags: tags
        .split(",")
    ...
    return (
      <div className="min-h-screen flex flex-col items-center justify-center text-white gap-4 text-center px-4">
        <p className="text-gray-400 max-w-md">
          You're signed in as an Attendee. Switch your account type to
          Organizer from your profile to host events.
        </p>
        <Link
          to="/profile"
          className="bg-violet-600 hover:bg-violet-500 px-6 py-3 rounded-xl font-semibold"
